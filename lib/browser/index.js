...
import { allManifest, fetchManifestMetadata, find, get, getAll, isVerbose, loadManifestFromApi, loadedVersion, setApiKey, setManifest, verbose, } from '../index.js';
...
export const getLatestCachedVersion = async () => {
    var _a, _b;
    const manifestsByVersion = (await idbKeys(manifestStore)).sort(compareVersionNumbers);
...
version in API: "${apiVersion}"`);
    ...
    // we already have the latest one cached but it's not loaded
        ...
        try {
            const cachedManifest = await idbGet(latestCachedVersion, manifestStore);
            if (!cachedManifest)
                throw new Error('fetching from indexeddb failed');
            setManifest(cachedManifest);
            isVerbose && `manifest loaded from indexeddb. ${Object.keys(allManifest !== null && allManifest !== void 0 ? allManifest : {}).length} components`;
            latestIsLoaded = true;
        }
        catch (e) {
            console.log(e);
